...
ASN1String & ASN1String::operator=(const ASN1String & s) {
  OCTET_STRING_fromString(ptr, s.str().c_str());
  return *this;
}
...
bool ASN1Oid::operator>(const ASN1Oid & other) const {
  ...
  while (it1 != end() and it2 != other.end()) {
...
bool ASN1Oid::operator==(const ASN1Oid & other) const {
  ...
  return it2 == other.end() and it1 == end();
}
